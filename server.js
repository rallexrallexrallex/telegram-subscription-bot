...
// Инициализация SQLite базы (файл на сервере)
...
app.get('/', (req, res) => {
    res.send(`
        ...
        <p>Статус: <strong>✅ Работает с SQLite</strong></p>
        <p>Канал: @${CONFIG.CHANNEL_USERNAME}</p>
        <p>Webhook URL для Lava.Top:</p>
        <code>${req.protocol}://${req.get('host')}/lava-webhook</code>
        <p>👇 Скопируйте эту ссылку в настройки Lava.Top</p>
    `);
});
...
async function addToChannel(userId, firstName) {
    try {
        ...
        await sendMessage(userId,
            ...
            `💎 Спасибо за подписку!\n\n` +
            `⏰ Срок доступа: ${calculateEndDateText(firstName)}`
        );
...
function calculateEndDateText(planName) {
    switch(planName) {
        case '1month':
            return '1 месяц';
        case '6months':
...
app.listen(PORT, () => {
    ...
    console.log(`💾 База: SQLite (/tmp/subscriptions.db)`);
